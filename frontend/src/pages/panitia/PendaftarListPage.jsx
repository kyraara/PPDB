    ...
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Users size={24} color="var(--accent-primary)" /> Daftar Pendaftar
        </h1>
      </motion.div>
        ...
        <div className="filter-bar" style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            ...
            <select
              ...
              style={{ ...inputStyle, padding: '0.65rem 0.75rem', minWidth: '160px', cursor: 'pointer', appearance: 'auto' }}
            >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              ...
              <tbody>
                {data.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: 'background 0.15s', cursor: 'pointer' }}
                    ...
                    >
                      ...
                      <td style={{ padding: '0.75rem', fontWeight: 500 }}>
                        <Link to={`/panitia/pendaftar/${p.id}`} style={{ color: 'var(--text-primary)', textDecoration: 'none' }}>{p.nama}</Link>
                      </td>
                      <td style={{ padding: '0.75rem', fontFamily: 'monospace', fontSize: '0.78rem', color: 'var(--text-secondary)' }}>{p.nomor_daftar}</td>
                      <td style={{ padding: '0.75rem', color: 'var(--text-secondary)' }}>{p.gelombang || '-'}</td>
                      <td style={{ padding: '0.75rem' }}>
                        <span style={{ display: 'inline-flex', padding: '0.2rem 0.6rem', borderRadius: '50px', fontSize: '0.7rem', fontWeight: 600, color: 'white', background: badge.bg, whiteSpace: 'nowrap' }}>
                          {badge.label}
                        </span>
                      </td>
