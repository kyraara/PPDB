        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-16 items-center">
          ...
          <div className="text-center lg:text-left flex flex-col items-center lg:items-start">
            ...
            <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              ...
              <strong className="text-accent-light dark:text-dark-accent-light">TK, SD, SMP, dan SMA</strong>.
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}
            ...
            {jenjangData.map((j, i) => {
              ...
              return (
                <motion.div key={j.key} initial="hidden" animate="visible" custom={i} variants={fadeInUp}
                  className="p-6 text-center rounded-lg backdrop-blur-2xl
                             bg-[rgba(26,46,26,0.85)] border border-[rgba(45,138,107,0.3)]">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3
                                  bg-accent-bg dark:bg-dark-accent-bg">
                    <j.icon size={24} className="text-[#3DAF84]" />
                  </div>
                  <h4 className="text-[1.05rem] mb-1">{j.key}</h4>
                  <p className="text-xs text-text-secondary dark:text-dark-text-secondary mb-3">{j.label}</p>
                  <div className="p-2 rounded-sm bg-[rgba(45,138,107,0.15)] border border-[rgba(45,138,107,0.2)]">
                    <div className="text-[1.75rem] font-bold text-[#3DAF84]">
